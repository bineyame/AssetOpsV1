...
class TestNoUpdateOrDeleteRouteExists:
    ...
    def test_no_per_site_route_is_registered_yet(self, client: TestClient) -> None:
        """Site Details addressed by `site_id` is the next slice."""
        schema = client.get("/openapi.json").json()

        assert [path for path in schema["paths"] if "{site_id}" in path] == []
        assert client.get(f"{SITES_PATH}/MG-002").status_code == 404
