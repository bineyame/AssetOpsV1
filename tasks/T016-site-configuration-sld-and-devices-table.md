# T016 - Site Configuration SLD And Devices Table

Status: planned
USER_REVIEW_REQUIRED: true

Intended branch: `task/T016-site-configuration-sld-and-devices-table`
...
Site Configuration renders the configured Single Line Diagram using the hybrid
mini-grid SLD view model and renders a Devices & Sensors table from the same
canonical Foundation source. SLD labels, ratings, configured signal
availability, and device rows agree because they are derived from the same
validated topology, device, and mapping facts.
...
while operational evidence remains unavailable.
...
- Site Configuration renders a `Single Line Diagram (Configured)` panel for a
  compatible hybrid mini-grid Site using the T015 SLD view model. The diagram
  renders configured topology, component labels, ratings, connections, and
  declared signal availability from the Foundation.
- The Devices & Sensors table renders from the same validated Foundation facts
  as the SLD: devices, mapped components, signal IDs, display labels, units,
  protocol metadata where declared, and sample cadence where declared.
- SLD labels and Devices & Sensors rows agree on component names, device names,
...
- For incompatible topology, the SLD panel renders an explicit unavailable
  state with a stable, accessible reason. It lists or links to the affected
  configured facts where useful, but it does not hide unmatched assets, render a
  partial diagram as complete, or fall back to a generic graph.
- The Devices & Sensors table still renders for an incompatible SLD if the
...
- Site Configuration remains ungated and renders identically with
  `simulator_lab.enabled` true and false, except for no simulator-specific
  runtime controls because none are rendered here.
- No operator navigation item is added. The surface remains reached from a Site.
...
- Device-to-signal mapping: UI + unit test.
  Devices & Sensors and SLD signal availability come from the same declared
  mappings.
...
- Shared Site presentation substrate: CI guard.
  Site Configuration remains in `frontend/src/sites/**` and shell-neutral.
...
- UI test asserting SLD labels and Devices & Sensors rows agree on names,
  ratings, units, and signal availability from the same fixture.
...
- UI test rendering an incompatible topology and asserting the explicit
  ...
  diagram.
- UI test asserting the Devices & Sensors table still renders valid device and
  mapping configuration when only the SLD archetype compatibility fails.
...
- UI test asserting Site Configuration renders with `simulator_lab.enabled`
  true and false and no operator navigation item is added.
...
Devices & Sensors, and topology/device/signal representation.
...
- The Devices & Sensors wording for configured devices, signal availability,
