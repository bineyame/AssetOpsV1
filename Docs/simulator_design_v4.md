...
v4 incorporates the Architect verification review of v3. It keeps the ten
items that review confirmed as closed, resolves the two remaining design gaps,
and folds the review's smaller implementation clarifications into the normative
sections rather than leaving them as side notes.

Where the review found that a lexical guard was weaker than the semantic
decision behind it, v4 follows the semantic decision. Where v3 introduced a new
comparison mechanism whose proof grain was too coarse, v4 specifies the
comparison over resolved frozen inputs rather than opaque version identifiers.
...
| Control vocabulary decision | Preserve both the lexical guard and the semantic decision behind it. Rename physical states, causes and records where legitimate, but **do not introduce renamed switching-position or control-mode enums**. Those concepts remain deferred until an explicit topology/evidence contract admits them. |
...
| Policy ownership | Baseline policy remains Site Foundation / Controls. `ControlAssumption` stays documentary. Typed component control properties land with property work; site-scoped control properties are deferred to the controller phase as a separate typed Site Controls collection. |
...
| Counterfactual pairing | Keep immutable `PairedExperiment`, outside run identity, but prove comparability over **resolved effective frozen inputs** (including control properties, initialization inputs, mappings and observation bindings), not version identifiers. |
...
- **T021A** remains in the current queue at its existing position after T021.
  It is an independent parser narrowing, not a prerequisite for the kernel.
...
These examples are intentionally limited to component-local physical state or
availability. Switching position and controller operating mode are **not**
admitted as scenario/configuration enums by renaming them; see §5.
...
**Keep both the lexical guard and the semantic decision behind it.**

Renaming is appropriate only when the renamed concept is independently truthful
as a physical state, an authored cause/action, or an operational record. It is
**not** appropriate when renaming would merely smuggle in the same switching
position or control mode that the product previously declined to model.

Examples that are valid now:

| Avoided spelling | Valid current concept |
| --- | --- |
| `door-open-excursion` | `door-ajar-excursion` — physical door state |
| `manual-tank-dip` | `operator-hand-dip` — operational record |
| `manual-override` | `operator-commanded-override` — authored cause/action |

Examples that remain **deferred rather than renamed**:

- breaker/power-path position as a closed configuration/scenario enum;
- controller mode such as automatic/manual as a closed configuration/scenario enum;
- fault-isolated/tripped switching position asserted by scenario configuration.

### Why this distinction matters

The existing parser guard is lexical, but the underlying decision is semantic:
a document must not assert a switching/control condition that the evidence and
topology model cannot yet substantiate.

Therefore:

- rename a door state because the simulator physically models the door;
- rename an operator record because the record itself is observable;
- name an operator-commanded intervention because the scenario is authoring a cause;
- **do not** rename `OPEN/CLOSED/TRIPPED/AUTO/MANUAL` into synonyms merely to
  clear CI.

If a later slice genuinely needs switching position or controller operating
mode, that slice must explicitly add the topology/evidence contract that makes
the state truthful and may then revise the vocabulary decision deliberately.
...
The component property carrier belongs with the typed component-property work.

Component-addressed binding is a separate extension layered on that carrier;
because both currently accumulate in T020A, the Planner must size/split that
work before implementation rather than treating all of it as one review-sized
slice.

Site-scoped control properties need a parallel typed Site Controls collection,
not an abuse of the component property table. They are **not required by
T020A** and should land with the first controller work, when source merit order
or critical-load priority is actually consumed.
...
### Run identity and intervention materialization
...
instant. Scenario-authored changes are already captured by the frozen scenario
binding.

For **run-scoped interventions that are not authored in the scenario**, an
identifier alone is insufficient unless it reconstructs immutable content.
Therefore every run-scoped intervention must be materialized before execution
as an immutable, canonically serialized intervention artifact whose identifier
is content-addressed.

Conceptually:

```text
FrozenInterventionArtifact
- intervention_ref = content digest of canonical payload
- intervention_kind
- effective_time
- target StateRef / property path
- before/after or operation payload
- provenance
```

`DeterministicIdentity.intervention_history` may remain `tuple[str, ...]`
provided each string is such an immutable content-addressed reference and the
referenced artifact is persisted with the run.

Thus reproducibility is:

```text
Foundation version
+ frozen scenario binding
+ ordered content-addressed intervention refs
= reproducible effective policy and run-scoped changes
```

This preserves the existing identity shape while ensuring an intervention
cannot change behind the identifier.
...
- shared_effective_input_projection_digest
- created_from
- status
```
...
## 15.3 Proving "all else equal" at the correct grain

Comparability is **not** proved by hashing run identities after removing a
Foundation or Scenario version identifier. Version identifiers are opaque
summaries: excluding one version field could hide several unrelated changes.

The pair builder instead compares a canonical projection of the **resolved
effective frozen inputs** used by each run.

The projection includes, as applicable:

- resolved `FrozenInitializationInput` entries keyed by `StateRef`;
- resolved component control properties;
- resolved site-scoped control properties;
- forcing/profile identities and resolved forcing parameters;
- observation bindings;
- publication profile and cadence bindings;
- signal mappings;
- topology/component property values that affect execution;
- numeric-policy version;
- model/kernel/execution-contract versions;
- seed and stochastic-stream contract;
- ordered frozen intervention artifacts.

Repository helpers such as `frozen_inputs()` should be reused where they
already produce the per-field/per-answerer representation rather than inventing
a second projection model.

The declared intervention delta identifies the **exact field/path or frozen
intervention artifact** allowed to differ.
...
intervention effective value:
BAT-001.minimum-reserve-soc = 32%

allowed delta path:
component/BAT-001/control/minimum-reserve-soc
```

The builder removes only that exact path/value pair from the equality
projection and compares everything else.

If any other effective input differs, the pair is rejected as
`NOT_COMPARABLE`.

### Why versions remain in provenance but not as the proof grain

Foundation and scenario versions remain essential provenance. They tell the
user which authored artefacts produced the run.

They are not sufficient evidence of all-else-equal comparison because:

```text
Foundation v12 → v13
```

may contain one changed reserve threshold or ten unrelated property changes.

Therefore `PairedExperiment` records both:
- the original run/version provenance; and
- the resolved-input comparison digest used to prove comparability.

## 15.4 More complex interventions
...
Represent this as an explicit frozen intervention artifact wherever possible,
rather than as an opaque version bump.

The paired comparison may then declare the allowed delta as:

```text
LOAD_ADDITION
target = LOAD-MILL-02
rated_power = 12 kW
active_window = 10:00–15:00
```

and still verify that all unrelated resolved inputs are identical.

If a comparison intentionally changes a wider bundle — for example a complete
Foundation redesign — it may still be paired for presentation, but its
`comparison_kind` must reflect a **multi-change scenario comparison** and it
must not claim single-variable/all-else-equal status.
...
NOT_COMPARABLE          # paired-experiment effective-input comparison
```
...
### T020A — sizing checkpoint before implementation

v4 places three related but separable concerns near T020A:

- the per-component typed property carrier already expected by T020A;
- component-addressed Foundation binding (`StateRef`, `FoundationBinding`,
  scenario state refs, frozen initialization);
- component-scoped typed control properties that use that carrier.

This is now enough responsibility that the **Planner must size or split the work
before implementation starts**.

The natural dependency cut is:
...
The second depends on the first; the first can be reviewed independently.
Whether the task identifiers change is the Planner's decision, but the design
must not force both into one unreviewable slice.

Site-scoped control properties are explicitly excluded from this immediate
work and belong with the first controller slice.

### T020B

T020B owns the boundary sampling semantics in §6. Its task size does not need to
grow; one declared semantic is replaced by the reconciled boundary-cycle rule.

### T021 / T021A ordering

T021A is **not** a prerequisite to T021. Keep the repository queue ordering and
the task-file rationale:

```text
T021  kernel work
then
T021A independent parser narrowing
```

The kernel does not depend on `execution_requirement` on a reported
observation, so moving T021A earlier adds no value.
...
v4 does not redefine those as nine monolithic implementation tasks.
...
13. `PairedExperiment` rejects every undeclared **resolved effective-input** difference, even when Foundation/Scenario versions differ;
...
## V4-01 — Vocabulary decision corrected beyond the lexical guard

**v3:** correctly preserved the guard, but still renamed switching positions and
control modes into unbanned synonyms.

**v4:** valid renaming is limited to physical states, authored causes/actions and
records. Switching-position and controller-mode enums remain deferred.
...
**Removed/deferred:**
- `CONDUCTING / ISOLATED / FAULT_ISOLATED` as power-path position;
- renamed AUTO/MANUAL mode enums.

**Justification:** the decision behind the guard is semantic, not lexical.
...
## V4-04 — Run-scoped intervention history made reconstructible

Every run-scoped intervention must be materialized as an immutable
content-addressed artifact before execution. The existing
`intervention_history: tuple[str, ...]` may remain because each string identifies
the canonical intervention payload itself.

**Justification:** a bare arbitrary identifier cannot reproduce a change that
exists nowhere else.
...
## V4-02 — Paired comparison proof moved to resolved effective inputs

**v3:** added the correct `PairedExperiment` mechanism but described equality as
a projection of frozen identities minus the declared delta.

**v4:** comparisons are performed over resolved effective frozen inputs at exact
field/path grain. Version identifiers remain provenance, not the equality proof.

**Justification:** excluding an opaque Foundation/Scenario version could conceal
unrelated changes inside the version bump.
...
## V4-03 — Build alignment refined

- T020A now carries enough adjacent responsibility to require a Planner sizing
  checkpoint before implementation; the natural cut is property carrier first,
  component addressing second.
- Site-scoped control properties move to controller-phase work rather than
  T020A.
- T021A remains after T021 in the current queue and is not a kernel
  prerequisite.

---

# 27. Architect-review integration notes

The v3 verification review found ten previously raised items closed and two
remaining gaps. v4 treats those findings as design input rather than as a
parallel review document.

## 27.1 Items intentionally unchanged because review verified them

The following v3 decisions remain normative:

- boundary sampling reconciliation in §6;
- component-addressed `StateRef`;
- documentary `ControlAssumption` plus typed control properties;
- BLAKE2b RNG contract;
- `EXACT_RATIONAL` numeric policy;
- declared-cascade reasoning and solver boundary;
- discrete-state initialization/trace/observation contract;
- scenario-owned dynamic initial state with explicit `UNKNOWN` stress;
- canonical source-envelope boundary;
- private Lab observations through `LabProjection`.

## 27.2 Immediate planning implication

The only immediate delivery risk identified by the review is task shape around
T020A. That is a planning/sizing concern, not a reason to weaken the design.

The Planner should explicitly preserve reviewability by separating, if needed:

1. typed component property representation;
2. component-addressed resolution/binding;
3. later controller-phase site-scoped control policy.

## 27.3 No programme-size change

These clarifications do not add a new product programme beyond v3. They move
already-implied work to the correct contract boundaries and prevent two classes
of false confidence:

- passing a lexical vocabulary guard while violating its semantic decision;
- claiming all-else-equal counterfactuals using opaque version identifiers.
